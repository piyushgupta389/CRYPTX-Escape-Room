...
function startGame(difficulty) {
  ...
  // Generate a fresh random password for each run
  if (difficulty === 'easy') {
    const hour = randomDigit();
    const minute = randomDigit();
    const page = randomDigit();
  ...
  if (difficulty === 'medium') {
    const d1 = randomDigit();
    const d2 = randomDigit();
    const d3 = randomDigit();
    const d4 = randomDigit();
    puzzle.password = `${d1}${d2}${d3}${d4}`;

    const paintingObj = puzzle.objects.find(o => o.id === 'painting');
    paintingObj.examineText = `Three shapes on the canvas: △ □ ◯. Below them: "The third is the key." △ is the third shape listed alphabetically... or is it the third in sequence? The number ${d1} is hidden in the frame.`;
    paintingObj.clue = `CLUE: The third shape is △ — triangle = ${d1}. First digit: ${d1}.`;

    const safeObj = puzzle.objects.find(o => o.id === 'safe');
    safeObj.examineText = `The safe screen shows: "${d2}_${d4}" — two of three digits visible. One key on the keypad glows brighter than the rest: the "${d3}". "Middle digit completes the pattern," reads a tag attached to the handle.`;
    safeObj.clue = `CLUE: Safe screen shows ${d2}_${d4}. The glowing key is ${d3}. Pattern: ${d2} [${d3}] ${d4} → digits 2,3,4 = ${d2}${d3}${d4}.`;

    const mirrorObj = puzzle.objects.find(o => o.id === 'mirror');
    mirrorObj.examineText = `The message on the mirror reads (in reverse): "ORDER IS: SHAPE, SAFE THREE, SAFE ONE." This tells you the digit sequence, not the digits themselves.`;
    mirrorObj.clue = `CLUE: The order confirmed — painting, then safe digits (${d2}, ${d3}, ${d4}). Full code: ${d1}${d2}${d3}${d4}.`;

    const noteObj = puzzle.objects.find(o => o.id === 'note');
    noteObj.examineText = `The note says: "The code is built from a shape, then a safe. Keep the order: ${d1}, ${d2}${d3}${d4}. If you're reading this — you're on the right track."`;
    noteObj.clue = `CLUE: Shape = ${d1}. Safe = ${d2}${d3}${d4}. Code = ${d1}${d2}${d3}${d4}.`;
  }
  ...
  if (difficulty === 'hard') {
    ...
    const clockObj = puzzle.objects.find(o => o.id === 'clock');
    clockObj.clockH = d1 * 30;
    clockObj.clockM = d2 * 30;
    clockObj.examineText = `Hour hand: position ${d1}. Minute hand: position ${d2} (on the clock face, pointing at the ${d2}). But is this a time, or a cipher? The label on the clock's back reads: "H then M."`;
    clockObj.clue = `CLUE: H=${d1}, M=${d2}. First two digits: ${d1}, ${d2}.`;

    const mirrorObj = puzzle.objects.find(o => o.id === 'mirror');
    mirrorObj.mirrorText = `${d2}`;
    mirrorObj.examineText = `The mirror has the digit ${d2} etched into its surface. "What digit looks the same reflected?" — the inscription beneath asks. ${d2}. This is also the second digit from the clock.`;
    mirrorObj.clue = `CLUE: ${d2} is confirmed as the second digit. Cross-reference clock: ${d1}, ${d2}, ?, ?.`;

    const drawerObj = puzzle.objects.find(o => o.id === 'drawer');
    const letter = digitToLetter(d3);
    drawerObj.examineText = `Underneath the drawer, carved: "${letter}". The alphabet uses A=1, B=2, C=3... so the digit is ${d3}.`;
    drawerObj.clue = `CLUE: ${letter} = ${d3}. Third digit is ${d3}. Running sequence: ${d1}, ${d2}, ${d3}, ?.`;

    const candleObj = puzzle.objects.find(o => o.id === 'candle');
    candleObj.examineText = `The candle has ${d4} hardened wax drips on the holder. The spiral at the base has ${d4} turns. "Count the turns and you have the final digit."`;
    candleObj.clue = `CLUE: ${d4} wax formations and turns. Final digit: ${d4}. Code = ${d1}${d2}${d3}${d4}.`;
  }
